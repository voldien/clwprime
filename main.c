...
int main(int argc, char** argv){
	...
	for(x = 0; x < nDevices; x++){
		clGetDeviceInfo(devices[x], CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(kerplat[x].globalworksize), &kerplat[x].globalworksize, &ret);
	...
	for(x = 0; x < nDevices; x++){
		...
		err = clEnqueueNDRangeKernel(kerplat[x].queue, kernel, 1, NULL/*&localworksize[0]*/, &workgroup[0], 0, 0, 0, 0);
		if(err != CL_SUCCESS){
	...
	/*	Fetch result.	*/
	for(x = 0; x < nDevices; x++){
		err = clEnqueueReadBuffer(kerplat[x].queue,
				buf,
				CL_TRUE, workoffset * sizeof(cl_uint),
				kerplat[x].maxXworkitems * sizeof(cl_uint),
				&PI[workoffset], 0, NULL, NULL);
		workoffset += kerplat[x].maxXworkitems;
		if(err != CL_SUCCESS){
